...
module.exports = function modulesLoader(beyo, options) {
  ...
  return new Promise(function (resolve, reject) {
    var modulesPath = path.join(beyo.rootPath, options.path);

    fs.access(modulesPath, fs.R_OK, function (err) {
      if (err) {
        //reject(ModuleLoaderException('Invalid modules path', { error: err }));
        beyo.emit('modulesNotFound', { path: modulesPath });
        resolve({});  // no modules
      } else {
...
function loadModules(beyo, options, files) {
  ...
  for (i = 0, iLen = files.length; i < iLen; ++i) (function (i) {
    ...
    try {
      moduleData.module = require(modulePath);
  ...
  if (moduleData === undefined) {
    ...
    moduleData.promise = loadModuleDependencies(beyo, options, moduleData.module.dependencies, availableModules, loadStack)
      ...
      .then(function (config) {
        var modulePath = path.join(beyo.rootPath, moduleData.path, moduleData.module.main || path.sep);

        moduleData.context = moduleContext(moduleData.module, config);

        beyo.emit('moduleLoad', moduleData);

        return Promise.resolve( require(modulePath) ).then(function (moduleInit) {
            return moduleInit.call(moduleData.context, beyo);
          }).then(function (initResult) {
            moduleData.initResult = initResult;

            return modelsLoader(beyo, {
                path: path.join(moduleData.path, 'models'),
                moduleName: moduleData.module.name || moduleName,
          ...
          .then(function () {
            beyo.emit('moduleLoaded', moduleData, availableModules);
          })
        ;
      })
      .then(function () {

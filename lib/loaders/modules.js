...
Options
  ...
  - modulePaths (optional)  an option specifying the models, controllers and services directories
                            will default to "models", "controllers" and "services" respectively.
...
module.exports = function * modulesLoader(beyo, options) {
  ...
  for (i = 0, iLen = files.length; i < iLen; ++i) {
    ...
    try {
      eventData.module = beyo.appRequire(path.join(beyo.appRoot, file));

      yield loadModule(beyo, options, eventData, []);
    } catch (e) {
...
function * loadModule(beyo, options, eventData, loadStack) {
  if (eventData.module.loaded) {
    return;
  } else if (!MODULE_NAME_REGEX.test(eventData.module.name)) {
    throw ModuleLoaderException('Invalid or missing module name: ' + String(eventData.module.name));
  }
...
/**
Return the module name from the dirname
*/

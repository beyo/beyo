...
module.exports = function * modulesLoader(beyo, options) {
  ...
  files = yield glob(MODULES_GLOB, { cwd: path.resolve(beyo.rootPath, options.path) });
  ...
  for (i = 0, iLen = files.length; i < iLen; ++i) {
    ...
    try {
      module = beyo.appRequire(path.join(beyo.rootPath, options.path, files[i]));
      module.__dirname = path.join(beyo.rootPath, options.path, moduleDir);
    } catch (e) {
    ...
    if (module) {
      ...
      if (!('name' in module)) {
        ...
        beyo.emit('moduleLoadError', ModuleLoaderException('Module name must be a string at: ' + path.join(options.path, moduleDir)), { moduleName: module.name });
...
function * loadModule(beyo, options, moduleName, availableModules, loadStack) {
  ...
  try {
    ...
    modulePath = path.relative(beyo.rootPath, module.__dirname)

...
function * appLoader(beyo, options) {
  ...
  var appInitModulePath = path.join(beyo.rootPath, 'app', 'index');
  var appInitModule;
  var app;
  var subApps = {};

  if (beyo.app) {
    throw ApplicationException('Application already defined!');
  }
  ...
  Object.defineProperties(beyo, {
   'createSubApp': {
      ...
      value: function (key, mountPath, baseApp) {
        if (subApps[key]) {
          throw ApplicationException('Sub-application already defined `' + key + '`');
        } else if (key === '*') {
          throw ApplicationException('Invalid Sub-application key `*`');
        }

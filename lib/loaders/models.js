...
module.exports = function * modelsLoader(beyo, options) {
  ...
  } else if (!options.context || options.context.__proto__.constructor !== Object) {
  ...
  for (i = 0, iLen = files.length; i < iLen; ++i) {
    try {
      eventData.modelName = getModelName(files[i]);
      ...
      if (eventData.initResult) {
        models[eventData.modelName] = eventData.initResult;
      }
    } catch (e) {

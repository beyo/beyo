...
require('util').inherits(Beyo, require('promise-events'));
    ...
    'init': {
      ...
      value: function init() {
        return initPromise || (initPromise = initApplication(beyo, options).then(function () {
          return beyo.emit('ready', beyo).then(function () {
            initializing = false;
            ready = true;
          });
        }).catch(function (e) {
...
function initApplication(beyo, options) {
  ...
  return beyo.emit('beforeInitialize', beyo).then(function () {
    return applicationMain(beyo, options);
  }).then(function () {
    ...
    return loggerLoader(beyo, beyo.config('logger', {})).then(function (logger) {
    ...
    //  console.error("**** beyo.js: app error", err);
    //})
    ;
  }).then(function () {
    return initPlugins(beyo, options);
  }).then(function () {
    return initHMVC(beyo, options);
  }).then(function () {
    // At this point, we don't care for async processes.. everything should be loaded
    // and everything should handle their own dependencies
    return beyo.emit('afterInitialize', beyo);
  });
}
...
function initHMVC(beyo, options) {
  ...
  Object.defineProperty(getModel, 'available', {
    configurable: false,
    enumerable: true,
    get: function getAvailableModels() {
      return modelKeys.slice(0);
    }
  });
  ...
  Object.defineProperty(getService, 'available', {
    configurable: false,
    enumerable: true,
    get: function getAvailableServices() {
      return serviceKeys.slice(0);
    }
  });
  ...
  beyo.on('moduleLoaded', function (evt) {
    moduleKeys.push(evt.module.name);
  });
...
function initDefaultOptions(options) {
  ...
  if (fs.existsSync(beyoConfPath)) {
    options = merge(true, require(beyoConfPath), options);
  }
...
/**
If the application contains main.js at it's root, is executed. The main module
should export a single function retunning immediatly, or a Promise instance. Any
resolved value is ignored.
*/
function applicationMain(beyo, options) {
  var mainPath = path.join(beyo.rootPath, 'main.js');

  return new Promise(function (resolve) {
    fs.stat(mainPath, function (err, stat) {
      if (err) {
        resolve(false);
      } else {
        resolve(stat.isFile());

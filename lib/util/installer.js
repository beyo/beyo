...
function * install(options) {
  ...
  if (options.basePath && options.definition) {
    try {
      def = require(path.join(fixturePath, options.definition));
    } catch (e) {
      throw "Definition file : " + path.join(fixturePath, options.definition) + "\n" + e.stack;
    }

    yield createFsStruct(options.basePath, def.structure, options.context || {});
  }

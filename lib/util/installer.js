...
module.exports.downloadStub = downloadStub;
...
function * install(options) {
  var fixturePath = path.join(path.dirname(path.resolve(process.cwd(), options.stub)), 'boilerplate');
  var defFile = fixturePath + '.json';
  var def;
  ...
  if (options.basePath && options.definition) {
    try {
      ...
      throw "Definition file : " + defFile + "\n" + e.stack;
    }

    yield createFsStruct(fixturePath, options.basePath, def.structure, options.context || {});
  }
...
function * shellExec(execCommands) {
  ...
  if (execCommands) {
    logger.log('debug', 'Executing shell commands');

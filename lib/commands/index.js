...
function bindOutputEvents(beyo, options) {
  ...
  beyo.on('pluginLoad', function (evt) {
    if (options.verbose && !options.quiet) {
      console.info('[Plugins]', 'Loading', evt.pluginName, '(' + (evt.pluginPath || evt.pluginModule) + ')');
    }
  });
  ...
  // ####### core
  beyo.on('serverStarted', function (port, host) {
    if (options.verbose && !options.quiet) {
      console.info('[*]', 'Listening on ' + (host || 'localhost') + ':' + port);
